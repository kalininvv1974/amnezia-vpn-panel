...
## Panel: флаг страны в карточке сервера (2026-09-23, НЕ опубликовано)
- В карточке Server (Location) добавлен флаг страны: `/api/geo` возвращает country + code,
  из ISO-кода собирается флаг-эмодзи (`countryFlag()`), фолбэк — название страны без флага.
- Изменены `index.html` + пересобран `PANEL_B64` в установщике
  (`embedded == index.html: True`, 98376 символов); `node --check` OK, 12 тестов логики флага OK.
- **На GitHub не выкладывалось** (по просьбе пользователя); когда разрешит — коммит и пуш.
