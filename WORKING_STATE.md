...
- В карточке Server (Location) рядом со страной показывается флаг. `/api/geo` возвращает
  country + code; из ISO-кода строится `<img>` (flagcdn.com, `flagImg()`).
- Флаги-эмодзи на Windows не рисуются (видны буквы «PL») → заменены на картинки.
- **Размер флага фиксирован 28x21** — фолбэк-SVG (jsdelivr/lipis) без размера иначе
  рисуется на всю ячейку. CSS width/height на `<img>` ограничивает любой источник.
  `alt=""` — при недоступности флага не показывается текст «PL», остаётся страна.
- **Шрифт значений карточки уменьшен**: `.info-value` 1.7rem → **1.3rem**.
- **`flagWatch()`**: источник переключается по таймеру 4 c, если не загрузился
  (не ждём десятки секунд таймаута браузера); `onerror="window.__flagStep(this)"`
  переключает сразу при ошибке. Цепочка: flagcdn w40 → flagpedia 72x54 → jsdelivr SVG →
  скрыть (остаётся страна). Скрипт в IIFE, поэтому `flagStep`/`flagWatch` выложены в
  `window.__flagStep`/`__flagWatch` для инлайн-обработчиков.
- **Карточка рисуется сразу** (страна из установки/сервера), флаг подставляется в ячейку
  Location асинхронно, когда `/api/geo` ответит (таймаут гео — 6 c); `preconnect` для
  flagcdn/flagpedia/jsdelivr в `<head>`.
- Баг (исправлен локально): `flagcdn.com/w24` — размера нет, 404; размер исправлен на w40.
- Изменены `index.html` + пересобран `PANEL_B64` в установщике
  (`embedded == index.html: True`, 103324 симв.); `node --check` OK, 29 тестов логики флага OK.
- Опубликовано на GitHub: `eefb130` (эмодзи), `1484c4e` (картинки flagcdn). Текущие
  правки размера/таймера/async-гео — локально, пуш только по разрешению пользователя.
...
- **Скриншоты пересняты с флагом страны** (Location: флаг Польши + «Poland»): headless
  Edge по мок-серверу с `/api/geo → {country: Poland, code: PL}`; флаг проверен по DOM
  (img без `display:none`) и по пикселям (838/297 красных пикселей — красная полоса флага).
- Файлы переименованы в `panel-dark-v2.png`/`panel-light-v2.png` (новый URL сбрасывает
  кеш превью GitHub, старые показывались из кеша).
